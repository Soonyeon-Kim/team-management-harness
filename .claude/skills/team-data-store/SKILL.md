...
├── calendar/
│   └── holidays.json       # 공휴일 (가동률 분모=가용일 계산용)
├── tasks/
...
- **가동률 (기간 기반, 컨설팅 표준 — 보고 KPI)** = 기간 내 **가동일 ÷ 가용일 × 100**.
  - **가동일** = 각 투입의 `(기간 ∩ [start,end])` 안의 실근무일(영업일 − 공휴일) × `allocation/100`을 합산한다. **고객 대상 활동(프로젝트·교육·행사)을 모두** 가동으로 보고, `status`가 `ended`여도 기간에 걸치면 집계한다(투입은 *시점*이 아니라 *기간*으로 본다). 같은 날 여러 투입이 겹치면 합산돼 100%를 넘을 수 있다(과투입).
  - **가용일** = 영업일(월~금) − 승인 휴가(leave.json `status==approved`, 종류 무관 `days` 합) − 공휴일(calendar/holidays.json). 휴가·공휴일과 겹치는 날은 가동일에서도 제외한다.
  - 보고 기본 기간은 **YTD(연초~기준일)**. 월·분기도 동일 식으로 산출한다. `days`(휴가)가 0.5/0.25면 그만큼 가용일을 분수로 차감한다.
- **현재 동시 투입률 (스태핑용, 옛 '가동률')** = 한 팀원의 `status == active` 투입 `allocation` 합. 투입 *등록 시점*에 100% 초과면 과투입 경고로 쓴다. "지금 묶인 capacity"라서, 기간 가동률(위)과는 다른 지표다 — 혼동하지 마라.
- `member`는 `members/`에 존재하는 slug여야 한다. 종료된 투입도 삭제하지 않고 `ended`로 보존(이력·가동률 기간 집계·역량 매칭 근거).

### calendar/holidays.json (공휴일 — 가동률 분모용)
```json
{
  "country": "KR",
  "notes": "가용일 계산용. 매년 팀이 검증·갱신.",
  "years": {
    "2026": [
...
```
- `years.{연도}`: 해당 연도의 공휴일 배열(`date` ISO, `name` 표시용). 근로자의 날(5/1) 등 회사가 쉬는 유급휴일도 포함한다.
- **용도**: 가동률의 가용일 = 영업일 − 승인 휴가 − 이 공휴일. 주말과 겹치는 공휴일은 어차피 영업일에서 빠지므로 중복 차감되지 않는다.
- 대체공휴일·음력 명절은 매년 바뀐다. 해당 연도 데이터가 없으면 공휴일 미반영으로 계산하고 그 사실을 보고한다(임의 추정 금지).
