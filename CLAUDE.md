...
| 2026-06-19 | 포크 후 실데이터 사용 가이드 추가(`docs/fork-guide.md`): PII/gitignore 안전(public repo — 실데이터는 `team-data/` 재ignore 또는 private), 데모 데이터 비우고 `checklist-setup`으로 본인 팀 초기화, Claude Code로 현행화, upstream은 `.claude`·`docs`·`CLAUDE.md`만 선별 sync 안내 | docs/fork-guide.md | 가상 데모로 공개된 repo를 포크해 실데이터로 운영하려는 사용자 대응 |
